...
public class ZoomListener {
     ...
     * @param group
     */
    public static void reset(Group group) {
        group.setScaleX(1.0);
        group.setScaleY(1.0);
    }
    /**
    ...
    public static ScrollPane createZoomPane(final Group group) {
        ...
        final ObjectProperty<Bounds> boundsProperty = new SimpleObjectProperty<Bounds>();
        scroller.viewportBoundsProperty().addListener(new ChangeListener<Bounds>() {
            @Override
            public void changed(ObservableValue<? extends Bounds> observable,
                                Bounds oldValue, Bounds newValue) {
                // 修复控件反复跳动的问题
                Bounds prevOldValue = boundsProperty.get();
                if (prevOldValue != null && !prevOldValue.equals(newValue)) {
                    zoomPane.setMinSize(newValue.getWidth(), newValue.getHeight());
                }
                boundsProperty.set(oldValue);
            }
        });
